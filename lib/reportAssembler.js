 ...
 */
function buildTableOfContents(studyData, hasPhotographs) {
  let sectionNum = 1;
  const toc = [];
...
function buildExhibitC(studyData) {
  ...
  return {
    ...
    methodologyNote:
      "Component costs are determined using recognized construction cost databases (RSMeans, Marshall & Swift), " +
...
export function assembleReport(studyData, wizardInputs = {}) {
  ...
  return {
    ...
    // ── Table of Contents ────────────────────────────────────────────
    tableOfContents: buildTableOfContents(studyData, photographs.length > 0),
    ...
    methodology: {
      ...
      costEstimationDescription:
        ...
        "cost databases (RSMeans and Marshall & Swift), regional cost adjustment factors for the " +
