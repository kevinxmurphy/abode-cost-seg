...
export async function sendPasswordReset({ to, name, resetUrl }) {
  ...
  const body = `
    ...
    <p style="margin:0 0 28px;font-size:15px;color:#666;text-align:center;line-height:1.6;">
      Hi ${displayName}, we received a request to reset your Abode password. Click the button below — this link expires in 1 hour.
    </p>
  ...
  try {
    ...
    if (error) {
      ...
      console.log("[email] Password reset sent:", data?.id, "→", to);
    }
  } catch (err) {
    console.error("[email] sendPasswordReset threw:", err.message);
  }
}
