...
// Lazy-initialize so the build doesn't throw when RESEND_API_KEY isn't present
// (env vars aren't available at Next.js build/static-analysis time)
function getResend() {
...
// ─── Welcome Email ───────────────────────────────────────────────────────────
/**
 * Sent when a new user creates an account (Google or email).
 *
...
export async function sendWelcomeEmail({ to, name }) {
  ...
  const quizUrl = "https://abodecostseg.com/quiz";
  ...
  const body = `
    ...
    <h1 style="margin:0 0 8px;font-size:24px;font-weight:800;color:#1a1a1a;text-align:center;letter-spacing:-0.5px;">
      Welcome to Abode, ${displayName}
    </h1>
    <p style="margin:0 0 28px;font-size:15px;color:#666;text-align:center;line-height:1.6;">
      You now have access to IRS-grade cost segregation studies at a fraction of the traditional cost.
    </p>

    <!-- What you get -->
    <table width="100%" cellpadding="0" cellspacing="0" style="background:#f9f7f4;border-radius:12px;padding:24px;margin-bottom:28px;">
      <tr><td>
        <p style="margin:0 0 14px;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.1em;color:#888;">Here's what Abode delivers</p>
        ${[
          "Full MACRS cost segregation study — PDF + Excel",
          "IRS Audit Techniques Guide methodology",
          "5-year, 7-year, 15-year, and 27.5-year asset classification",
          "Ready to hand to your CPA at tax time",
        ].map((text) => `
        <table cellpadding="0" cellspacing="0" style="margin-bottom:8px;">
          <tr>
            <td style="width:22px;vertical-align:top;color:${BRAND_TEAL};font-size:14px;font-weight:700;">&#x2713;</td>
            <td style="padding-left:8px;font-size:14px;color:#444;line-height:1.5;">${text}</td>
          </tr>
        </table>`).join("")}
      </td></tr>
    </table>
    ...
    <p style="margin:0;font-size:13px;color:#999;text-align:center;line-height:1.6;">
      Takes about 2 minutes. No payment required to see your estimate.
    </p>
  `;
  ...
  try {
    const { data, error } = await resend.emails.send({
      from: FROM,
      to,
      subject: "Welcome to Abode — your cost seg study starts here",
      html: emailShell(body),
    });

    if (error) {
      log.error("[email] sendWelcomeEmail failed:", error);
    } else {
      log.info("[email] Welcome email sent:", data?.id, "→", to);
    }
  } catch (err) {
    log.error("[email] sendWelcomeEmail threw:", err.message);
  }
}
...
export async function sendPurchaseConfirmation({ to, name, propertyAddress, sessionId }) {
  const resend = getResend();
  if (!resend) {
    log.warn("[email] RESEND_API_KEY not set — skipping purchase confirmation");
    return;
  }
  ...
  try {
    ...
    if (error) {
      log.error("[email] sendPurchaseConfirmation failed:", error);
    } else {
      log.info("[email] Purchase confirmation sent:", data?.id, "→", to);
    }
  } catch (err) {
    log.error("[email] sendPurchaseConfirmation threw:", err.message);
  }
}

// ─── Share Study with CPA ────────────────────────────────────────────────────
/**
 * Sent to a CPA when a user shares their study from the properties page.
 *
 * @param {{ to: string, cpaName: string, fromName: string, propertyAddress: string, firstYearDeduction: number, taxSavings: number, studyType: string, message: string }} opts
 */
export async function sendStudyShareCPA({
  to,
  cpaName,
  fromName,
  propertyAddress,
  ...
  const resend = getResend();
  if (!resend) {
    log.warn("[email] RESEND_API_KEY not set — skipping CPA share");
    return;
  }

  const displayCPA = cpaName ? cpaName.split(" ")[0] : "there";
  const displayFrom = fromName || "Your client";
  const displayAddress = propertyAddress || "their property";
  const isCatchUp = studyType === "catch-up";
  const fmt = (n) => n?.toLocaleString?.() ?? "—";

  const body = `
    <h1 style="margin:0 0 8px;font-size:24px;font-weight:800;color:#1a1a1a;letter-spacing:-0.5px;">
      Hi ${displayCPA},
    </h1>
    <p style="margin:0 0 24px;font-size:15px;color:#666;line-height:1.6;">
      <strong style="color:#1a1a1a;">${displayFrom}</strong> has shared their cost segregation study with you via Abode.
    </p>

    <!-- Property -->
    <table width="100%" cellpadding="0" cellspacing="0" style="background:#f9f7f4;border-radius:12px;padding:24px;margin-bottom:24px;">
      <tr><td>
        <p style="margin:0 0 4px;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.1em;color:#888;">Property</p>
        <p style="margin:0 0 20px;font-size:16px;font-weight:700;color:#1a1a1a;">${displayAddress}</p>

        <table width="100%" cellpadding="0" cellspacing="0">
          <tr>
            <td width="50%" style="padding-bottom:4px;">
        ...
        <p style="margin:16px 0 0;font-size:12px;color:#888;line-height:1.5;">
          ⚠ This is a <strong>catch-up study</strong> (property acquired prior to 2023). An IRC §481(a) adjustment
          is included — Form 3115 filing is required to claim the catch-up deduction in the current tax year.
        </p>` : ""}
      </td></tr>
    </table>
    ...
    ${message ? `
    <!-- Personal note -->
    <table width="100%" cellpadding="0" cellspacing="0" style="border-left:3px solid ${BRAND_TEAL};padding:0 0 0 16px;margin-bottom:24px;">
      <tr><td>
        <p style="margin:0 0 4px;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.1em;color:#888;">Note from ${displayFrom}</p>
        <p style="margin:0;font-size:14px;color:#444;line-height:1.6;">${message}</p>
      </td></tr>
    </table>` : ""}

    <!-- What this study is -->
    <table width="100%" cellpadding="0" cellspacing="0" style="background:#f9f7f4;border-radius:12px;padding:20px 24px;margin-bottom:24px;">
      <tr><td>
        <p style="margin:0 0 12px;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.1em;color:#888;">About this study</p>
        <p style="margin:0;font-size:13px;color:#555;line-height:1.6;">
          This MACRS cost segregation study was prepared by Abode Cost Segregation and identifies
          personal property (5-year &amp; 7-year MACRS), land improvements (15-year MACRS), and Section 1250
          building property — enabling accelerated depreciation deductions under IRC §168.
        </p>
      </td></tr>
    </table>

    <!-- CTA -->
    <p style="margin:0 0 4px;font-size:13px;color:#999;text-align:center;line-height:1.6;">
      To download the full PDF study report, ask <strong style="color:#1a1a1a;">${displayFrom}</strong> to log in to their
      Abode dashboard and share the file directly, or contact
      <a href="mailto:hello@abodecostseg.com" style="color:${BRAND_TEAL};">hello@abodecostseg.com</a>.
    </p>
  `;
  ...
  try {
    const { data, error } = await resend.emails.send({
      from: FROM,
      to,
      subject: `Cost segregation study for ${displayAddress} — $${fmt(firstYearDeduction)} deduction`,
      html: emailShell(body),
    });

    if (error) {
      log.error("[email] sendStudyShareCPA failed:", error);
    } else {
      log.info("[email] CPA share email sent:", data?.id, "→", to);
    }
  } catch (err) {
    log.error("[email] sendStudyShareCPA threw:", err.message);
  }
}
...
export async function sendStudyComplete({ to, name, propertyAddress, firstYearDeduction, taxSavings, totalComponents }) {
  const resend = getResend();
  if (!resend) {
    log.warn("[email] RESEND_API_KEY not set — skipping study complete");
    return;
  }
  ...
  try {
    ...
    if (error) {
      log.error("[email] sendStudyComplete failed:", error);
    } else {
      log.info("[email] Study complete email sent:", data?.id, "→", to);
    }
  } catch (err) {
    log.error("[email] sendStudyComplete threw:", err.message);
  }
}
