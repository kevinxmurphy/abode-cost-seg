...
export async function sendWelcomeEmail({ to, name }) {
  ...
  try {
    ...
    console.error("[email] sendWelcomeEmail threw:", err.message);
  }
}
...
export async function sendStudyShareCPA({
  ...
  const resend = getResend();
  if (!resend) {
    console.warn("[email] RESEND_API_KEY not set — skipping CPA share");
    return;
  }
  ...
  try {
    ...
    if (error) {
      console.error("[email] sendStudyShareCPA failed:", error);
    } else {
      console.log("[email] CPA share email sent:", data?.id, "→", to);
    }
  } catch (err) {
    console.error("[email] sendStudyShareCPA threw:", err.message);
  }
}
