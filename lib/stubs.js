...
// PDF/CSV download — calls real API route or generates client-side
export async function downloadStudy(studyId, format) {
  if (format === "pdf") {
    // For stored studies, open the API route; for sample, POST with report_data
    if (studyId && studyId !== "sample") {
      window.open(`/api/study/pdf?id=${studyId}`, "_blank");
    } else {
      // Sample preview — generate from mock data via POST endpoint
      try {
        const { generateStudy } = await import("@/lib/reportEngine");
        ...
        const study = generateStudy({
          ...
          yearBuilt: 2018,
          isFurnished: true,
          materialParticipation: { hours: "500+", usesPropertyManager: "no" },
          wizardAnswers: {},
          taxBracket: 32,
        });

        const report = assembleReport(study, { photos: [], attestation: {}, amenities: [], renovations: [] });

        const res = await fetch("/api/study/pdf", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ report_data: report, preview: true }),
        });
        ...
        a.href = url;
        a.download = "Abode_Sample_Study_Preview.pdf";
        a.click();
        URL.revokeObjectURL(url);
      } catch (err) {
        console.error("Sample PDF generation failed:", err);
        alert("Sample PDF preview is temporarily unavailable. Start your free estimate to see a real study.");
      }
    }
    ...
    try {
      ...
      const csv = generateDepreciationCSV(study);
      downloadCSV(csv, "Abode_Sample_Depreciation_Schedule.csv");
    } catch (err) {
      console.error("Sample CSV generation failed:", err);
      alert("Sample schedule preview is temporarily unavailable. Start your free estimate to see a real study.");
    }
  }
}
