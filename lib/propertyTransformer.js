...
import { mapAmenitiesToCategories, parseDescription } from "./airbnbParser.js";
...
export function transformAirbnbData(raw) {
  ...
  // Feature-level enrichment: maps amenities to cost seg item keys (pool, bath-features, etc.)
  // Include title + description as extra signal so "heated pool" in listing title/body triggers pool detection
  const signalText = [raw.name || raw.title || "", raw.description || ""].filter(Boolean).join(" | ");
  const categoryEnrichment = mapAmenitiesToCategories(amenities, signalText);

  // Description hints: flooring, countertops, etc. detected from description text
  const descriptionHints = parseDescription(raw.description || "");
  ...
  return {
    ...
    // Feature-level enrichment for mergeAirbnbDefaults() — keys like pool, bath-features, deck-patio
    categoryEnrichment,
    // Description-derived hints (flooring, countertops, etc.)
    descriptionHints,
    // IRS depreciation year buckets (5yr/7yr/15yr) — separate from feature enrichment
    irsCategoryEnrichment: buildCategoryEnrichment(amenities),
    _raw: raw,
  };
}
