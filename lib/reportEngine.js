...
//   MAXIMIZED_ALLOCATIONS, MAXIMIZED_GUARDRAILS,
...
export const MACRS_TABLES = {
  ...
  /** Mid-quarter convention, Q4 — IRS Pub. 946, Table A-2 */
  fiveYearMQ4:  [5.00, 38.00, 22.80, 13.68, 10.94, 9.58],
  sevenYearMQ4: [3.57, 27.55, 19.68, 14.06, 10.04, 7.18, 7.18, 10.74],
  fifteenYearMQ4: [
...
/**
 * Maximized allocation percentages — higher reclassification within
 * IRS-defensible ranges. Matches what top-tier engineering firms produce.
 * For risk-tolerant owners with experienced CPAs.
 */
export const MAXIMIZED_ALLOCATIONS = {
  "sfr-furnished": {
    fiveYear: 24,
    sevenYear: 1.0,
    fifteenYear: 13,
...
/** Wider guardrails for maximized mode — still IRS-defensible */
export const MAXIMIZED_GUARDRAILS = {
 ...
 */
export function adjustAllocation(baseAllocation, wizardAnswers, guardrails) {
  const activeGuardrails = guardrails || RECLASSIFICATION_GUARDRAILS;
  ...
  if (fiveYear > activeGuardrails.fiveYear.warning) {
    warnings.push(`5-year allocation (${fiveYear.toFixed(1)}%) exceeds typical range. Capped at ${activeGuardrails.fiveYear.hardCap}%.`);
  }
  if (fiveYear > activeGuardrails.fiveYear.hardCap) {
    fiveYear = activeGuardrails.fiveYear.hardCap;
    guardrailsApplied = true;
  }

  if (sevenYear > activeGuardrails.sevenYear.warning) {
    warnings.push(`7-year allocation (${sevenYear.toFixed(1)}%) exceeds typical range. Capped at ${activeGuardrails.sevenYear.hardCap}%.`);
  }
  if (sevenYear > activeGuardrails.sevenYear.hardCap) {
    sevenYear = activeGuardrails.sevenYear.hardCap;
    guardrailsApplied = true;
  }

  if (fifteenYear > activeGuardrails.fifteenYear.warning) {
    warnings.push(`15-year allocation (${fifteenYear.toFixed(1)}%) exceeds typical range. Capped at ${activeGuardrails.fifteenYear.hardCap}%.`);
  }
  if (fifteenYear > activeGuardrails.fifteenYear.hardCap) {
    fifteenYear = activeGuardrails.fifteenYear.hardCap;
    guardrailsApplied = true;
  }
  ...
  if (totalReclass > activeGuardrails.totalReclass.warning) {
    warnings.push(`Total reclassification (${totalReclass.toFixed(1)}%) exceeds typical range of ${activeGuardrails.totalReclass.acceptable[0]}-${activeGuardrails.totalReclass.acceptable[1]}%.`);
  }
  if (totalReclass > activeGuardrails.totalReclass.hardCap) {
    // Scale down proportionally to fit under the hard cap
    const scale = activeGuardrails.totalReclass.hardCap / totalReclass;
    fiveYear = fiveYear * scale;
    sevenYear = sevenYear * scale;
    fifteenYear = fifteenYear * scale;
    totalReclass = fiveYear + sevenYear + fifteenYear;
    guardrailsApplied = true;
    warnings.push(`Total reclassification scaled to ${activeGuardrails.totalReclass.hardCap}% hard cap.`);
  }
...
export const COMPONENT_MASTER_LIST = [
  ...
  },
  {
    ...
    description: "Solar panels, inverters, monitoring systems, racking (owned systems only — leased not depreciable)",
...
export function distributeWithinClass(classTotal, macrsClass, wizardAnswers) {
  ...
  const rawW = wizardAnswers || {};

  // Normalize energy-features multi-select into individual fields
  const energyFeatures = Array.isArray(rawW["energy-features"]) ? rawW["energy-features"] : [];
  const w = {
    ...rawW,
    solar: rawW.solar || (energyFeatures.includes("solar") ? "owned" : undefined),
    generator: rawW.generator || (energyFeatures.includes("generator") ? "whole-house" : undefined),
  };
  ...
  const weightedComponents = components.map(comp => {
    ...
    switch (comp.id) {
      ...
      case "generator":
        ...
        else if (!w.generator || w.generator === "none") weight *= 0.01;
        break;

      case "central-vacuum":
        // Rare — keep default low weight unless explicitly noted
        break;

      case "septic-system":
        if (Array.isArray(w["water-system"]) && w["water-system"].includes("septic")) weight *= 2.0;
        else if (!w["water-system"]) weight *= 0.05;
        else weight *= 0.01;
        break;
...
function calculateCatchUp(depreciableBasis, allocPercents, monthPlaced, yearPlaced, currentYear, acquisitionDate) {
  ...
  const pisFallbackDate = new Date(yearPlaced, monthPlaced - 1, 15);
  const catchUpBonusRate = getBonusRate(
    pisFallbackDate,
    acquisitionDate || pisFallbackDate
  );

  // New method: what SHOULD have been claimed with cost seg (including bonus)
  ...
  // Bonus in PIS year applies to reclassified property only (not building)
  const fiveBonus = Math.round(fiveAmt * catchUpBonusRate);
  const sevenBonus = Math.round(sevenAmt * catchUpBonusRate);
  const fifteenBonus = Math.round(fifteenAmt * catchUpBonusRate);

  // Remaining basis after bonus runs through regular MACRS
  const fiveRemaining = fiveAmt - fiveBonus;
  const sevenRemaining = sevenAmt - sevenBonus;
  const fifteenRemaining = fifteenAmt - fifteenBonus;
  ...
  for (let i = 0; i < yearsHeld; i++) {
    ...
    // Year 1 includes bonus; subsequent years are regular MACRS on remaining basis
    const fiveYrBonus = i === 0 ? fiveBonus : 0;
    const sevenYrBonus = i === 0 ? sevenBonus : 0;
    const fifteenYrBonus = i === 0 ? fifteenBonus : 0;

    const fiveYr = fiveYrBonus + (i < fiveSched.length ? Math.round(fiveRemaining * fiveSched[i] / 100) : 0);
    const sevenYr = sevenYrBonus + (i < sevenSched.length ? Math.round(sevenRemaining * sevenSched[i] / 100) : 0);
    const fifteenYr = fifteenYrBonus + (i < fifteenSched.length ? Math.round(fifteenRemaining * fifteenSched[i] / 100) : 0);
...
export function generateStudy(inputs) {
  const {
    ...
    landValueSource = "estimate", // "county" | "market-estimate" | "user-override" | "appraisal"
  } = inputs;
  ...
  const allocTable = confidenceMode === "maximized"
    ? MAXIMIZED_ALLOCATIONS
    : BASE_ALLOCATIONS;
  const activeGuardrails = confidenceMode === "maximized"
    ? MAXIMIZED_GUARDRAILS
    : RECLASSIFICATION_GUARDRAILS;
  const baseAlloc = allocTable[allocType];
  ...
  // ── Adjust allocation ──────────────────────────────────────────────
  const adjusted = adjustAllocation(baseAlloc, enrichedAnswers, activeGuardrails);
  ...
  // Select correct rate tables based on convention (mid-quarter Q4 vs half-year)
  const fiveYearTable = useMidQuarter ? MACRS_TABLES.fiveYearMQ4 : MACRS_TABLES.fiveYear;
  const sevenYearTable = useMidQuarter ? MACRS_TABLES.sevenYearMQ4 : MACRS_TABLES.sevenYear;
  const fifteenYearTable = useMidQuarter ? MACRS_TABLES.fifteenYearMQ4 : MACRS_TABLES.fifteenYear;

  const fiveYearSched = buildClassSchedule(fiveYearAmt, fiveYearTable, bonusRate);
  const sevenYearSched = buildClassSchedule(sevenYearAmt, sevenYearTable, bonusRate);
  const fifteenYearSched = buildClassSchedule(fifteenYearAmt, fifteenYearTable, bonusRate);
